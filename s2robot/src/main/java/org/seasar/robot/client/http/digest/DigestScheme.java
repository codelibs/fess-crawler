...
/**
 * MT-safe DigestScheme implementation.
 * 
...
public class DigestScheme extends RFC2617Scheme {
     ...
     */
    private String createDigest(final Credentials credentials)
            throws AuthenticationException {
        ...
        StringBuilder tmp =
            new StringBuilder(uname.length() + realm.length() + pwd.length()
                + 2);
        tmp.append(uname);
        tmp.append(':');
        tmp.append(realm);
        ...
        if (algorithm.equalsIgnoreCase("MD5-sess")) {
            ...
            String tmp2 =
                encode(digester.digest(EncodingUtils.getBytes(a1, charset)));
            StringBuilder tmp3 =
                new StringBuilder(tmp2.length() + nonce.length()
                    + cnonce.length() + 2);
            tmp3.append(tmp2);
            tmp3.append(':');
            tmp3.append(nonce);
        ...
        String hasha2 =
            encode(digester.digest(EncodingUtils.getAsciiBytes(a2)));

        // 3.2.2.1
        String serverDigestValue;
        if (qopVariant == QOP_MISSING) {
            StringBuilder tmp2 =
                new StringBuilder(hasha1.length() + nonce.length()
                    + hasha1.length());
            tmp2.append(hasha1);
            tmp2.append(':');
            tmp2.append(nonce);
        ...
        String serverDigest =
            encode(digester.digest(EncodingUtils
                .getAsciiBytes(serverDigestValue)));
    ...
    private Header createDigestHeader(final Credentials credentials,
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            boolean noQuotes =
                "nc".equals(param.getName()) || "qop".equals(param.getName());
