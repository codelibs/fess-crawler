...
public class SmbClient extends AbstractCrawlerClient {
    ...
    protected volatile SmbSessionHolder smbSessionHolder = null;

    @Override
    public synchronized void init() {
        if (smbSessionHolder != null) {
            return;
        }
        ...
        final SmbSessionHolder holder = new SmbSessionHolder();
    ...
    public void destroy() {
        ...
        smbSessionHolder.forEach(session->session.disconnect());
        smbSessionHolder = null;
    }
    ...
    protected ResponseData getResponseData(final String uri, final boolean includeContent) {
        ...
        final SmbSession smbSession = smbSessionHolder.get(fineUri);
        ...
        try {
            ...
            final List<String> pathComponents = new LinkedList<>(Arrays.asList(decodedPath.split("/")));
            pathComponents.remove(0);
            shareName = pathComponents.remove(0);
            path = String.join("\\", pathComponents);
            if (logger.isDebugEnabled()) {
                logger.debug("UNC shareName: " + shareName + ", path: " + path);
            }
        ...
        Session session;
        try {
            session = smbSession.getSession();
        } catch (final IOException ioe) {
            if (logger.isDebugEnabled()) {
                logger.debug("Cannot get session: " + uri, ioe);
            }
            throw new CrawlingAccessException("Cannot get session: " + uri, ioe);
        }

        try (final DiskShare share = (DiskShare) session.connectShare(shareName);
                final DiskEntry entry = share.open(path, EnumSet.of(AccessMask.GENERIC_READ), null, SMB2ShareAccess.ALL,
            ...
            if (entry == null) {
                ...
                final FileStandardInformation fsi = fai.getStandardInformation();
                ...
                if (fai.getStandardInformation().isDirectory()) {
                    ...
                    if (includeContent) {
                        final List<FileIdBothDirectoryInformation> list = share.list(path);
                        for (final FileIdBothDirectoryInformation fdi : list) {
                            final String fileName = fdi.getFileName();
                            if (fileName.equals(".") || fileName.equals("..")) {
                                continue;
                            }
                            if (!StringUtil.endsWithIgnoreCase(fineUri, "/")) {
                                fineUri = new StringBuilder(fineUri).append("/").toString();
                            }
                            final String childUrl = new StringBuilder(fineUri)
                                    .append(URLEncoder.encode(fileName, "UTF-8").replace("+", "%20")).toString();
                            requestDataSet.add(RequestDataBuilder.newRequestData().get().url(childUrl).build());
                        }
                    }
                    ...
                    responseData.setLastModified(fbi.getLastWriteTime().toDate());
                    responseData.addMetaData(SMB_CREATE_TIME, fbi.getCreationTime().toDate());

                    final RPCTransport transport = SMBTransportFactories.LSASVC.getTransport(session);
                    final LocalSecurityAuthorityService lsas = new LocalSecurityAuthorityService(transport);
                    final PolicyHandle policyHandle = lsas.openPolicyHandle();

                    final EnumSet<SecurityInformation> si = of(SecurityInformation.OWNER_SECURITY_INFORMATION);
                    final SecurityDescriptor sd = file.getSecurityInformation(si);
                    final SID ownerSid = sd.getOwnerSid();
                    if (ownerSid != null) {
                        final com.rapid7.client.dcerpc.dto.SID sid = new com.rapid7.client.dcerpc.dto.SID(ownerSid.getRevision(),
                                ownerSid.getSidIdentifierAuthority(), ownerSid.getSubAuthorities());
                        final String[] accountNames = lsas.lookupNamesForSIDs(policyHandle, sid);
                        final PolicyDomainInfo pdi = lsas.getPolicyPrimaryDomainInformation(policyHandle);
                        if (logger.isDebugEnabled()) {
                            logger.debug("SMB accountName: " + accountNames[0] + ", domainName: " + pdi.getDomainName());
                        }
                        final String[] ownerAttributes = { accountNames[0], pdi.getDomainName() };
                        responseData.addMetaData(SMB_OWNER_ATTRIBUTES, ownerAttributes);
                    }

                    processAccessControlEntries(responseData, file, smbSession);

                    // TODO add file meta data

                    if (EnumWithValue.EnumUtils.isSet(fai.getAccessInformation().getAccessFlags(), AccessMask.FILE_READ_DATA)) {
                        ...
                        if (includeContent) {
                            ...
                            if (fsi.getEndOfFile() < maxCachedContentSize) {
                                try (final InputStream contentStream = new BufferedInputStream(file.getInputStream())) {
                            ...
                            try (final InputStream is = responseData.getResponseBody()) {
                                responseData.setMimeType(mimeTypeHelper.getContentType(is, file.getFileInformation().getNameInformation()));
                            } catch (final Exception e) {
                                responseData
                                        .setMimeType(mimeTypeHelper.getContentType(null, file.getFileInformation().getNameInformation()));
                            }
                        } else {
                            if (logger.isDebugEnabled()) {
                                logger.debug("SMB MIME Type: " + fineUri);
                        ...
                        if (contentLengthHelper != null) {
                            ...
                            if (responseData.getContentLength() > maxLength) {
                                throw new MaxLengthExceededException("The content length (" + responseData.getContentLength()
                                        + " byte) is over " + maxLength + " byte. The url is " + fineUri);
                            }
                        }
                    } else {
                        // Forbidden
    ...
    protected void processAccessControlEntries(final ResponseData responseData, final com.hierynomus.smbj.share.File file,
            final SmbSession smbAuthentication) {
        ...
        try {
            final LocalSecurityAuthorityService lsas =
                    new LocalSecurityAuthorityService(SMBTransportFactories.LSASVC.getTransport(smbAuthentication.getSession()));
            final PolicyHandle policyHandle = lsas.openPolicyHandle();
            final EnumSet<SecurityInformation> si = of(SecurityInformation.DACL_SECURITY_INFORMATION);
            final SecurityDescriptor sd = file.getSecurityInformation(si);
            if (logger.isDebugEnabled()) {
            ...
            if (acl != null) {
                ...
                for (final com.hierynomus.msdtyp.ace.ACE e : acl.getAces()) {
                    final com.rapid7.client.dcerpc.dto.SID sid = new com.rapid7.client.dcerpc.dto.SID(e.getSid().getRevision(),
                            e.getSid().getSidIdentifierAuthority(), e.getSid().getSubAuthorities());

                    final String[] accountNames = lsas.lookupNamesForSIDs(policyHandle, sid);
                    // Translate SID to get SID_NAME_USE
                    final com.rapid7.client.dcerpc.dto.SID[] sids = lsas.lookupSIDsForNames(policyHandle, accountNames);
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("ACE Type: " + ace.getType() + ", AccountName: " + ace.getAccountName());
                    }
                    aces.add(ace);
                }
                responseData.addMetaData(SMB_ACCESS_CONTROL_ENTRIES, aces.toArray(new ACE[aces.size()]));
            }
        } catch (final SMBApiException | IOException e) {
            logger.warn("Could not get ACE " + file.toString(), e);
            throw new CrawlingAccessException("Could not get ACE " + file.toString(), e);
        }
    }
    ...
    private void copy(final com.hierynomus.smbj.share.File src, final File dest) {
        if (dest.exists() && !dest.canWrite()) {
            return;
        }
        try (BufferedInputStream in = new BufferedInputStream(src.getInputStream());
    ...
    public void setSmbAuthenticationHolder(final SmbSessionHolder smbSessionHolder) {
        this.smbSessionHolder = smbSessionHolder;
    }
}
