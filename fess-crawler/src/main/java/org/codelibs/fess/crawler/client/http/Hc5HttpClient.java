...
public class Hc5HttpClient extends AbstractCrawlerClient {
    ...
    protected DnsResolver dnsResolver = new Hc5IdnDnsResolver();
    ...
    */
    ...
    public synchronized void init() {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {}", Hc5HttpClient.class.getName());
        }
        ...
        // Set response timeout (equivalent to socket timeout in HC4)
        if (soTimeoutParam != null) {
            requestConfigBuilder.setResponseTimeout(Timeout.ofMilliseconds(soTimeoutParam));
        }
        ...
        formSchemeList.forEach(p -> {
            final Hc5FormScheme scheme = p.getFirst();
     ...
     * @param soTimeoutParam The socket timeout in milliseconds
     * @return The configured connection manager
     */
    protected PoolingHttpClientConnectionManager buildConnectionManager(final Integer connectionTimeoutParam,
        ...
        final PoolingHttpClientConnectionManagerBuilder builder = PoolingHttpClientConnectionManagerBuilder.create()
                ...
                .setConnectionTimeToLive(org.apache.hc.core5.util.TimeValue.of(timeToLive, timeUnit));
        ...
        // SSL
        if (getInitParameter(IGNORE_SSL_CERTIFICATE_PROPERTY, ignoreSslCertificate, Boolean.class)) {
            try {
     ...
     */
    public ResponseData doHttpMethod(final String url, final ClassicHttpRequest httpRequest) {
        if (httpClient == null) {
            init();
        }
    ...
    protected ResponseData processHttpMethod(final String url, final ClassicHttpRequest httpRequest) {
        ...
        try {
            // get a content
            final ClassicHttpResponse response = executeHttpClient(httpRequest);
            httpEntity = response.getEntity();
     ...
     */
    protected void closeResources(final ClassicHttpRequest httpRequest, final ResponseData responseData) {
        CloseableUtil.closeQuietly(responseData);
        if (httpRequest instanceof org.apache.hc.core5.concurrent.Cancellable) {
            ((org.apache.hc.core5.concurrent.Cancellable) httpRequest).cancel();
        }
    }
    ...
    private static final DateTimeFormatter RFC_1123_FORMATTER =
            DateTimeFormatter.ofPattern("EEE, dd MMM yyyy HH:mm:ss zzz", Locale.ENGLISH).withZone(ZoneId.of("GMT"));

    /**
     * Parses the last modified date from a string value.
     * Supports RFC 1123, RFC 1036, and ANSI C date formats.
    ...
    protected Date parseLastModifiedDate(final String value) {
        ...
        try {
            final Instant instant = Instant.from(RFC_1123_FORMATTER.parse(value));
