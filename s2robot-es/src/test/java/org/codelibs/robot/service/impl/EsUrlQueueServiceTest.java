...
public class EsUrlQueueServiceTest extends LastaDiTestCase {
    ...
    public void test_insert_update_deleteTx() {
        ...
        assertTrue(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "sessionId")).execute()
                .actionGet().exists());

        urlQueueService.delete("sessionId");
        assertFalse(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "sessionId")).execute()
                .actionGet().exists());
    ...
    public void test_insert_update_delete_multiTx() {
        ...
        assertTrue(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id1")).execute()
                .actionGet().exists());
        assertTrue(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id2")).execute()
                .actionGet().exists());

        urlQueueService.delete("id1");
        assertFalse(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id1")).execute()
                .actionGet().exists());
        assertTrue(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id2")).execute()
                .actionGet().exists());
        ...
        assertFalse(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id1")).execute()
                .actionGet().exists());
        assertFalse(esClient.prepareExists(".robot").setTypes("queue").setQuery(QueryBuilders.termQuery("sessionId", "id2")).execute()
                .actionGet().exists());
    }
}
