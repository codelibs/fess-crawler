...
public abstract class AbstractCrawlerService {
    ...
    protected void refresh() {
        ActionGetUtil.actionGet(getClient().admin().indices().prepareRefresh(index).execute());
    }
    ...
    protected void insert(final Object target, final OpType opType) {
        ...
        ActionGetUtil.actionGet(getClient().prepareIndex(index, type, id).setSource(source).setOpType(opType).setConsistencyLevel(writeConsistencyLevel)
    ...
    protected <T> void doInsertAll(final List<T> list, final OpType opType) {
        ...
        final BulkResponse bulkResponse = ActionGetUtil.actionGet(bulkRequest.setConsistencyLevel(writeConsistencyLevel).setRefresh(true).execute());
